...
pub struct TunConfig {
    ...
    // mihomo compat: auto-redirect fields
    #[serde(default)]
...
fn default_inet4_address() -> Vec<String> {
    // mihomo compat: uses FakeIP range addr with /30 prefix to create a tiny
    // connected route that doesn't interfere with nftables REDIRECT for the
    // full FakeIP /16 range. See mihomo config.go: tunAddressPrefix = PrefixFrom(addr, 30)
