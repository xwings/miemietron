...
/// Deserialize a value that might be either a number or a quoted string.
/// mihomo configs from subscriptions often have `interval: "3600"` instead of `interval: 3600`.
...
pub struct ProxyConfig {
    ...
    /// Set by global tcp-concurrent config (not per-proxy in YAML).
    #[serde(default)]
    ...
    /// mihomo compat: global keepalive settings injected by ProxyManager.
    #[serde(default)]
